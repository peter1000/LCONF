""" Example implementation: of the documentation: docs/source/main_docs/LCONF_Specification-4.1.rst: 'EXAMPLE 4 (JSON - LCONF)'
                                                                        using the: LCONF Repeated Mapping-Block and Key-Value-Mapping
"""
from inspect import (
   getfile,
   currentframe
...
def main():
   ...
   print('  result1.categories.PHP.tests.test1_name: ', type(result1['categories']['PHP']['tests']['test1_name']), result1['categories']['PHP']['tests']['test1_name'])
   ...
   for blk_name, blk_obj in result1['categories'].yield_key_value_order():
      ...
      print('        blk_obj.tests: ', type(blk_obj['tests']), blk_obj['tests'])
      # looping through the Mapping-Block-Items:
      for item_key, item_value in blk_obj['tests'].yield_key_value_order():
