...
def main():
   ...
   lconf__obj = lconf_prepare_and_parse_section(example_lconf_section_str, example_template, validate=False)
   lconf__obj__no_comments = lconf_remove_comments(lconf__obj)

   # EMIT PARSED  and dump it to file
   with open('example6c_dumpto_jsonfile.lconf', 'w') as file_:
      jdump(lconf__obj__no_comments, file_, indent=3)

   print('\n\n======== Finished dumping to file: file content: ========\n\n')

   with open('example6c_dumpto_jsonfile.lconf', 'r') as file_:
      print(file_.read())
