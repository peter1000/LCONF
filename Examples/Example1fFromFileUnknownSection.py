...
def main():
   ...
   with open(example_file, 'r') as file_:
      all_sections = lconf_extract_all_sections(file_.read())
      # loop through any 'LCONF-Sections': LCONF supports multiple sections
      for section in all_sections:
         # split the lines
         section_lines = section.splitlines()
         # get the section_name: we need it to check which section_template obj to use
         not_needed_start_tag, section_name = section_lines[0].split(' :: ', 1)
         if section_name == 'BaseEXAMPLE':
            result1 = lconf_prepare_and_parse_section_lines(section_lines, section_name, example_template)
