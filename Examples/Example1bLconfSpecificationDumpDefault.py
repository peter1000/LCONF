...
example_template = RdictFO2([
   ...
   ('#1', ''),
   ('#2', '# Just a Default Comment which can be emitted'),
   ('key1value_pair', ''),
   ('key2value_pair', ''),
   ...
   ('#3', ''),
   ('#4', '# Just a Default Comment which can be emitted: below a Main `Key-Value-Mapping'),
   ...
   # Main `Key-Value-Mapping: type: RdictFO2
   ('#5', ''),
   ('#6', '# Just a Default Comment which can be emitted: below a Main `Key-Value-Mapping'),
   ('key11value_mapping', RdictFO2([
      ('mapping11_key1', ''),
      ('#7', '# Just a Default Comment which can be emitted: below a Main key11value_mapping: item'),
   ...
   ('#8', ''),
   ('#9', '# Just a Default Comment which can be emitted: below a Repeated Block Identifier'),
   ('RepeatedBlk1', RdictIO([
      ...
      ('dummy_blk', RdictFO2([
         ...
         ('#10', ''),
         ('#11', '# Just a Default Comment which can be emitted: below a Block-MyKey1mapping'),
         ('MyKey1mapping', RdictFO2([
            ('blk_mapping_key1', ''),
            ('blk_mapping_key2', (0.0, lconf_to_float)),
            ('#12', '# Just a Default Comment which can be emitted: below a Block-MyKey1mapping: item'),
...
def main():
   ...
   lconf__obj = lconf_prepare_default_obj(example_template)
   result1 = lconf_parse_section(lconf__obj, example_lconf_section_str, example_template)

   emit_result = lconf_emit(result1, onelinelists=True)
   print('\n\n===  ===  === emit_result: ===  ===  === \n\n', emit_result)

   # EMIT PARSED / dump_default_comment
   with open('example1a_lconfspecification_dump_parsed_with_default_comments.lconf', 'w') as file_:
      result_emit__dump_default_no_comment = lconf_emit_default_obj(example_template, 'TestSection', onelinelists=True, emit_dummy_blks=True, with_comments=False)
      file_.write(result_emit__dump_default_no_comment)

   print('\n\n======== Finished dumping to file: file content: ========\n\n')

   with open('example1a_lconfspecification_dump_parsed_with_default_comments.lconf', 'r') as file_:
      print(file_.read())


   # EMIT / dump_default_comment
   with open('example1a_lconfspecification_dump_default_comments.lconf', 'w') as file_:
      result_emit__dump_default_comment = lconf_emit_default_obj(example_template, 'TestSection', onelinelists=True, emit_dummy_blks=True, with_comments=True)
      file_.write(result_emit__dump_default_comment)

   print('\n\n======== Finished dumping to file: file content: ========\n\n')

   with open('example1a_lconfspecification_dump_default_comments.lconf', 'r') as file_:
      print(file_.read())

   # EMIT / dump_default_comment
   with open('example1a_lconfspecification_dump_default_no_comments.lconf', 'w') as file_:
      result_emit__dump_default_no_comment = lconf_emit_default_obj(example_template, 'TestSection', onelinelists=True, emit_dummy_blks=True, with_comments=False)
      file_.write(result_emit__dump_default_no_comment)

   print('\n\n======== Finished dumping to file: file content: ========\n\n')

   with open('example1a_lconfspecification_dump_default_no_comments.lconf', 'r') as file_:
      print(file_.read())
