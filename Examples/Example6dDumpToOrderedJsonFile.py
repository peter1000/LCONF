...
from LCONF.MainCode import (lconf_prepare_and_parse_section, lconf_remove_comments)
...
def main():
   ...
   lconf__obj = lconf_prepare_and_parse_section(example_lconf_section_str, example_template)
   lconf__obj__no_comments = lconf_remove_comments(lconf__obj)

   # to_ordereddict
   result_to_ordereddict = lconf__obj__no_comments.to_ordereddict()
   # EMIT PARSED  and dump it to file
   with open('example6d_dumpto_ordered_jsonfile.lconf', 'w') as file_:
      jdump(result_to_ordereddict, file_, indent=3)

   print('\n\n======== Finished dumping to file: file content: ========\n\n')

   with open('example6d_dumpto_ordered_jsonfile.lconf', 'r') as file_:
      print(file_.read())
